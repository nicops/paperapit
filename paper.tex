...
Operations being done offline also means that there is no network latency to be worried of,
and can be more performant. Performance is important in VCS because if the revision
system is too slow, it might affect the workflow of the tool. Operations like commits, 
searching through the history, taking the diff out of two branches, and especially merging
if not done fast enough are limiting what you can do with it, the impact of this depending of 
...
the change is well tested and can be actually shared to everybody else, since the 
commit is going to be shared as soon as it's done, and could ``break the build''. 
In a DVCS the nature of the commits change, since commits are made in the local 
repository, they tend to be more atomic, 
...
At the same way, another co-worker can do Pull over my repo and get my new code & changes
